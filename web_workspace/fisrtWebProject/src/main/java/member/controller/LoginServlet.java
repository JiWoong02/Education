...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request 매개변수: 
		//사용자(브라우저, view 페이지) input 으로 입력된 값 또는 쿼리스트링으로 전달된 값을 가지고 오는 객체임
		//전송방식이 get 이면 request head 에 전송값이 기록되서 전송됨 => url 상에 보여짐
		//전송방식이 post 이면 request body 에 전송값이 인코딩돼서 저장되어 전송됨 => url 상에 안 보여짐
		...
		// 1. 전송온 값에 한글이 있다면 인코딩 처리함 //보통 로그인에 한글이 전송오지 않기때문에 굳이 작성할 필요없음
		request.setCharacterEncoding("UTF-8");	//view 페이지의 문자셋과 맞춰줌
		
		//2. 전송온 값 꺼내서 변수 또는 dto 객체에 저장 처리함 //매개변수는 여러개를 보내면 안좋음. 그래서 User 객체에 담아서 전달
		//String userId = request.getParameter("userid"); //view 의 input 의 name 속성의 이름으로 값을 꺼냄
		...
		User loginUser = new MemberService().selectLogin(user); //NULL 값이 오면 로그인 실패
		...
		if(loginUser !=null) {	//로그인 성공시
				...
				//뷰와 함께 같이 보내 정보(데이터)가 있을 때는, RequestDispatcher 를 사용함 
				//																	=> 상대경로만 사용할 수 있음
				//모든 컨트롤러(서블릿)의 실행 위치는 root (content directory : webapp) 에서 실행되고 있음
				
				//내보낼 뷰를 지정함 : 상대경로만 사용 가능함
				RequestDispatcher view = request.getRequestDispatcher("views/common/error.jsp");
				
				//뷰와 함께 같이 보낼 데이터 처리 : request 에 저장함 => Map 구조임 (key, value)
				request.setAttribute("message", "로그인 실패! 아이디와 암호를 다시 확인하세요");
				//key만 다르게해서 계속해서 setAttribute로 담을 수 있음
