# numpy_test2.py
# numpy 는 ndarray 클래스를 사용함 : C언어로 만든 내부로직을 제공함
...
for didx in range(0,len(thar)): # 면 반복 : 0, 1
    for ridx in range(0,len(thar[didx])): # 면 안의 행 반복 : 0, 1, 2
        for cidx in range(0, len(thar[didx][ridx])): # 면 안의 행 안의 열반복
...
print('1행 0열의 값 : ', tar[1][1], tar[1,0])
