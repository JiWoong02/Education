...
# numpy 에서 행과 열이 다른 백터끼리도 연산이 가능하도록 지원함 => 이 기능을 브로드 캐스팅이라고 함
# 크기가 다른 백터가 자동으로 크기가 큰 백터의 행과 열갯수가 맞춰짐 (확장)
...
print(np.median(x3))    # 1. 5   최대값과 최소값

print(np.all([True, True, False]))   # False
print(np.any([True, True, False]))  # 한개만 True

ar = np.zeros((100, 100), dtype=np.int_)
print(ar)
print(np.any(ar != 0)) # False, 배열 안에 0이 아닌게 하나라도 있느냐 ?
...
print(((a <= b) & (b <= c)).all())  # a가 b보다 모두 작거나 같으냐 & b가 c보다 모두 작거나 같으냐   , 둘 다 True냐?
# (a <= b) : [T, T, T, T]
# (b <= c) : [T, T, T, T]
# & : [T & T].all() => 모두 True냐?
...
# 2차원배열은 행렬(가로값들)로 정렬, 열별(세로값들)로 정렬을 함 (axis 인수 사용함)
