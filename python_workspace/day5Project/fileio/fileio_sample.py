...
열기모드:
w(wt), x(xt) : 새로쓰기
 - w(wt) : 대상 파일이 없으면 파일을 자동으로 새로 만들여서 열기함
...
def test_fwrite():
    ...
    f = open('testa.txt', 'w', encoding='utf-8')   # 덮어쓰기
    ...
    # open() 함수 첫번째 인자에 전체경로명과 파일명을 함께입력하면 됨
...
def test_writelines():
    ...
    f.writelines(ls)
    f.write('\n')
    f.writelines({'a':5, 'b':6, 'c':7})
    f.write('\n')
    
    # 각 아이템별로 한 줄씩 기록을 원하면, write() 반복 실행하면 됨
    for i in ls:
        f.write(i)  # 문자가 아닌 아이템은 str() 사용함
        f.write('\n')
    ...
    #                  마지막 라인을 읽고 나서, 더 이상 읽을 라인이 없으면 None 리턴함
...
def test_fread():
    ...
    # 파일의 내용을 한 줄씩 읽도록 처리한다면, None 이면
    while True:
        line = f.readline()
