...
def test_iterable():
    nlist = [1, 2, 3, 4]
    #iterator object : 리스트 처럼 순차적으로 값을 가진 객체를 말함
    # isinstance
...
def print_gugudan():
    su = int(input('수를 입력해주세요 : '))
    for dan in range(1,10):
        print(f'{su} * {dan} = {su*dan}')
...
# 리스트 안의 아이템 안의 값의 갯수가 동일할 경우에는 단순 for 문으로 해결 가능 가능함
def list_in_list():
    fruit_list = [['apple',10, 800], ['banana', 3, 2500], ['orange', 15, 500]]

    for fname, famount, fprice in fruit_list:
        print(f'과일이름 : {fname}, 과일갯수 : {famount}, 과일가격 : {fprice}')
