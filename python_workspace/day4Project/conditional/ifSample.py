...
# 변수 또는 값 in 군집자료형 변수 : x in s  => s 안에 x 가 있느냐?  => x가 있으면 True, 없으면 False 리턴함
...
# 결재 수단 중에 'money' 가 있으면, '5000원을 현금 지불하였습니다.' 출력하고,
...
def checkPayment():
    ...
    # if 'money' in payment:
    if 'bank' in payment:
        print('{}원을 현금 지불하였습니다.'.format(price))
    else :
        print('다른 결재 수단을 선택하세요 .')


def test_try():
    try: print(int(input('정수숫자 입력:')))
...
def checkPayment2():
    ...
    # if 'money' in payment:
    if no ==1:
        print('{0}원을 {1} 지불하였습니다.'.format(price, payment[1]))
    elif no ==2:
