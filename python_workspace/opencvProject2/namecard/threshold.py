...
# ..... > threshold namecard1.jpg 외부입력으로 실행시 (자바에서 파이썬 exe 파일 실행시에도 필요함)
if len(sys.argv) > 1: # 외부에서 들어온 전달값이 있느냐?   WHY? sys.argv[0] 은 실행파일명임 ( ex. threshold )
    filename = sys.argv[1]
...
# cv2.imshow('src small', src) # 이미지 크기 줄임
#src = cv2.resize(src,(0,0), fx=0.5, fy=0.5)  # 가로로 0.5배, 세로로 0.5배
# dsize 를 이미지 배율의 기준점 지정 (0, 0)으로 지정함, fx, fy : scaling factor (크기 배율) : 1.0 이 원래 크기, 축소 < 1.0 < 확대
cv2.waitKey()
cv2.destroyAllWindows()

# 히스토그램은 이미지의 밝기의 분포를 그래프로 표현할 수 있음
# 그래이스케일 채널수 1개 [0], BG 색상채널이 2개이면 [0,1], BGR 색상채널이 3개이면 [0, 1, 2] 로 색상정보가 표시될 것임
hist = cv2.calcHist([src],[0],None,[256],[0,256])
# 이미지 소스는 반드시 리스트 형태로 입력할 것 : [src]
# cv2.calcHist([images], channels, mask, histSize#색단계를 몇단계로 표현할건지?, ranges[, hist [, accumulate]])
