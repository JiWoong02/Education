...
# 오버레이 처리 함수
# 영상 프레임 이미지, 고양이귀 이미지, 고양이 귀 표시할 위치정보 전달받아서 오버레이해서 출력 처리함
def overlay(frame, cat, pos):
    if pos[0] < 0 or pos[1] < 0:  # 좌표값이 음수가 오면 안됨
        return

    if pos[0] + cat.shape[1] > frame.shape[1] or pos[1] - cat.shape[0] > frame.shape[0]:    # 프레임 크기를 벗어나면 안됨
        return
    ...
    img2 = cat[:, :, 0:3]   # 고양이 그림을 슬라이싱해서 저장해라 (고양이 그림 전체 선택임)
