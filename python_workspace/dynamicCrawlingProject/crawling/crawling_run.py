...
브라우저에 입력된 웹 사이트와 이 사이트의 검색 필드의 검색 키워드를
...
# 압축 풀어서 압축 푼 폴더 안에 exe 파일을 현재 프로젝트 폴더로 복사함
def run():
    # 크롬 드라이버 등록
    # Mac 용
    # driver = wd.Chrome('../chromedriver')
    # 윈도우용 (./ : 현재폴더, ../ : 상위폴더)
    driver = wd.Chrome(service=Service('./chromedriver.exe'))  # Service 클래스는 프로젝트가 기준  # 크롬 브라우저 실행
    ...
    # 로마 가볼만한 곳 글자 출력될 때까지 기더리게 설정 : #nxTsOv > div > div.MainContainer._travel_header.MainContainer-vjtko > div.PanelHeader-vpb6F > div.TabList-Erooo._scroll_wrapper > div.scroll-D9KKR._scroller > ul > li:nth-child(4) > a
    element_locator = (By.CSS_SELECTOR, '#nxTsOv > div > div.MainContainer._travel_header.MainContainer-vjtko > div.content-qIit7 > div.SummaryTopPoiContainer-qpzcR > div > div.head-xMGxp > div > h3')
    try:
        element = WebDriverWait(driver, timeout=10).until(EC.presence_of_element_located(element_locator))
    ...
    for item in item_list:
        #print(type(item.find_element(By.CLASS_NAME,'rank-shDNS')))
        rank  = item.find_element(By.CLASS_NAME,'rank-shDNS').text
    ...
    # 브라우저 종료
    driver.close()      # 크롬 부라우저 닫기
    driver.quit()       # 드라이버 종료

    return tour_list # main 으로 리턴 : 프로세스 종료
