...
class TourModel:
    ...
    def __init__(self):
        dbtemp.oracle_init()
        self.__conn = dbtemp.connect()      # 드라이버 등록 : 딱 한번 실행
    ...
    def insert_tour(self, tp_tour):
        ...
        try:
            ...
            with self.__conn.cursor() as cursor:        # 커서는 자동 close
                cursor.execute(query, tp_tour)
            dbtemp.commint(self.__conn)
        except Exception as msg:
            dbtemp.rollback(self.__conn)
            print('insert_tour 실패 : ', msg)
        finally:
            dbtemp.close(self.__conn)       # db는 직접닫기
            self.__conn = ''
    ...
    def select_all(self):
        ...
        try:
            # 자동 close 되게 처리 : with resource 문 이용
            with self.__conn.cursor() as cursor:        # 커서는 자동 close
                cursor.execute(query)       # 실행결과는 cursor안에 들어있음
    ...
    def select_one(self, tp_tour):
        ...
        try:
            # 자동 close 되게 처리 : with resource 문 이용
            with self.__conn.cursor() as cursor:  # 커서는 자동 close
                cursor.execute(query, tp_tour)  # 실행결과는 cursor안에 들어있음
