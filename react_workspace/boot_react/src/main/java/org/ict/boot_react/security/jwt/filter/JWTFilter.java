...
public class JWTFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(requestURI.startsWith("/boards")) {
            log.info("/boards 요청입니다.");
            filterChain.doFilter(request, response);
            return;
        }
        if("/favicon.ico".equals(requestURI)){
            log.info("/favicon.ico 요청입니다.");
            filterChain.doFilter(request, response);
            return;
        }

        log.info("이게 실행되면 안됩니다.");
        if ("/logout".equals(requestURI)) {
            //찾아갈 대상으로 그대로 넘김
            filterChain.doFilter(request, response);
            //SecurityConfig에 설정된 LogoutHandler 를 상속받아 만든 CustomLogoutHandler 가 작동됨
            return;
        ...
        if (authorization == null || !authorization.startsWith("Bearer")) {
            filterChain.doFilter(request, response);
            //SecurityConfig 에 설정된 permitAll() 에 등록된 url이면, 해당 컨트롤러 메서드로 전달됨
            //토큰이 필요한(로그인해야 요청되는) 요청은 에러를 클라이언트에게 보냄
            return;
        }

        // Bearer 토큰에서 JWT를 추출함 (토큰 정보가 request헤더에 있는 경우)
        // authorization이 가진 값 : "Bearer 토큰문자열"
        ...
        //MemberDto(회원정보) 객체를 기반으로 CustomUserDetails 객체를 생성합니다.
        //Security 가 제공하는 UserDetails 를 상속받은 인증용 클래스로 만든 것임 : security.jwt.model.CustomUserDetails
        ...
        //액세스 토큰(로그인 상태 확인)이 이상이 없다면 처리할 내용임
        
        //필터 체인을 계속 진행함
        filterChain.doFilter(request, response);    //실제 컨트롤러로 요청을 넘기면됨
