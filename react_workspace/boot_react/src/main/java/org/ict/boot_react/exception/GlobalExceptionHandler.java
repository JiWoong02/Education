...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(DataAccessException.class)// 이 애플리케이션에서 해당 exception이 발생하면 자동실행
    public ResponseEntity<String> handleDataAccessException(DataAccessException ex){
        //데이터베이스 접근 중 발생하는 모든 예외를 처리하는 메서드임
        // 주로 CRUD작업 실패 시 발생하는 예외임
    ...
    @ExceptionHandler(Exception.class)// 모든 익셉션
    ...
    @ExceptionHandler(MethodArgumentNotValidException.class)// 모든 익셉션
    public ResponseEntity<Object> handleValidationException(MethodArgumentNotValidException ex){
        ...
        ex.getBindingResult().getAllErrors().forEach((error) -> {
            ...
            String errorMessage = error.getDefaultMessage();    //필드에서 발생한 오류 메세지
            errors.put(fieldName, errorMessage);       // 필드명과 오류 메세지를 맵에 추가
        });

        // 유효성 검사 실패에 대한 상세 정보를 담은 Map 을 클라이언트에게 반환함
